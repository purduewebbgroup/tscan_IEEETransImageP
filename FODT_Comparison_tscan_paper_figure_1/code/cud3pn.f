...
c
      ...
      complex(kind=2) cof(nx,ny,nz,8),cofxy(nx,ny,6)
	...
	do i=1,nx
     ...
     +                                 cof(i,j,k,6)*phi(i,j,k+1))
      ...
      complex(kind=2) phi(0:nx+1,0:ny+1,0:nz+1),phxy(0:nx+1,0:ny+1)
      if (ixy.eq.0) then
	do j=0,ny+1
	  do i=0,nx+1
...
c
      ...
      complex(kind=2) cof(nx,ny,nz,8),cofxz(nx,nz,6)
      complex(kind=2) phi(0:nx+1,0:ny+1,0:nz+1)
      j = jset
      do k=1,nz
	do i=1,nx
     ...
     +                                 cof(i,j,k,4)*phi(i,j+1,k))
      ...
      complex(kind=2) phi(0:nx+1,0:ny+1,0:nz+1),phxz(0:nx+1,0:nz+1)
      if (ixz.eq.0) then
	do k=0,nz+1
	  do i=0,nx+1
...
c
      ...
      complex(kind=2) cof(nx,ny,nz,8),cofyz(ny,nz,6)
      complex(kind=2) phi(0:nx+1,0:ny+1,0:nz+1)
      i = iset
      do k=1,nz
	do j=1,ny
     ...
     +                                 cof(i,j,k,2)*phi(i+1,j,k))
      ...
      complex(kind=2) phi(0:nx+1,0:ny+1,0:nz+1),phyz(0:ny+1,0:nz+1)
      if (iyz.eq.0) then
	do k=0,nz+1
	  do j=0,ny+1
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      real(kind=2) fparm,xa,xb,yc,yd,tolmax,relmax
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      complex(kind=2) tx(nx,ny,*),ty(ny,nx,*),cof(nx,ny,6),sum(*)
      common/icup2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
...
c
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      complex(kind=2) rhsc(ncx,ncy),resf(nx,ny)
      complex(kind=2) phi(0:nx+1,0:ny+1),phic(0:ncx+1,0:ncy+1)
      complex(kind=2) cof(nx,ny,6)
c
c     set phic zero
c
      do jc=0,ncy+1
	do ic=0,ncx+1
	  phic(ic,jc) = (0.0D0,0.0D0)
	end do
      end do
c
...
c
      implicit none
      integer nx,ny
      complex(kind=2) phi(*),cof(*),tx(*),ty(*),sum(*)
      integer intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      common/icup2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
...
c
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      complex(kind=2) phi(0:nx+1,0:ny+1),cof(nx,ny,6)
c
...
c
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      complex(kind=2) phi(0:nx+1,0:ny+1),cof(nx,ny,6),tx(nx,ny,*),
     +sum(ny)
c
...
c
	...
	do i=1,nx
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      complex(kind=2) phi(0:nx+1,0:ny+1),cof(nx,ny,6),ty(ny,nx,*),
     +sum(nx)
c
