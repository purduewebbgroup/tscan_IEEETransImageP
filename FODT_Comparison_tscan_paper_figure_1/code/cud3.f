...
c
     ...
     +        klevel,kcur,kps
      ...
      complex(kind=2) phif(nx,ny,nz),rhsf(nx,ny,nz),wk(*)
      real(kind=2) phmax
      common/icud3/intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
...
c
	  if (phmax.gt.0.0D0) relmax = relmax/phmax
...
c
     ...
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      complex(kind=2) phi(0:nx+1,0:ny+1,0:nz+1),phic(0:ncx+1,0:ncy+1,0:
     +ncz+1)
      complex(kind=2) rhsc(ncx,ncy,ncz),resf(nx,ny,nz),cof(nx,ny,nz,8)
...
c
      ...
      complex(kind=2) cof(nx,ny,nz,8)
      complex(kind=2) tx(nx,ny,nz,*),ty(ny,nx,nz,*),tz(nz,nx,ny,*),wk(*)
      integer intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      real(kind=2) xa,xb,yc,yd,ze,zf,tolmax,relmax
     ...
     +kcycle,iprer,ipost,intpol
      ...
      common/cud3c/kpbgn(50),kcbgn(50),ktxbgn(50),ktybgn(50),
     +ktzbgn(50),nxk(50),nyk(50),nzk(50),ngrid,klevel,kcur,kps
      real(kind=2) dlx,dly,dlz,dlx2,dly2,dlz2,dlxx,dlyy,dlzz,cmin,cemax,
     +alfmax
      complex(kind=2) cxx,cyy,czz,cx,cy,cz,ce,alfa,gbdy,x,y,z,c1,c2,
     +c3,c4,c5,c6
      real(kind=2) cxxr,cxxi,cyyr,cyyi,czzr,czzi
...
c
	...
	do j=jst,jfn
	  ...
	  do i=ist,ifn
	    ...
	    czzr = dble(czz)
	    czzi = dimag(czz)
	    cmin = dmin1(cmin,dmax1(cxxr*cyyr*czzr,cxxi*cyyi*czzi))
	    cemax = dmax1(abs(ce),cemax)
c
c     flag hyperbolic pde & adjust coeffs if necessary
c
	    if (abs(cx)*dlx*0.5D0 .gt. abs(cxx)) then
	      if (klevel.eq.ngrid) ier = -4
	      cxx = complex(abs(cx)*dlx*0.5D0,0.0D0)
	    end if
	    if (abs(cy)*dly*0.5D0 .gt. abs(cyy)) then
	      if (klevel.eq.ngrid) ier = -4
	      cyy = complex(abs(cy)*dly*0.5D0,0.0D0)
	    end if
	    if (abs(cz)*dlz*0.5D0 .gt. abs(czz)) then
	      if (klevel.eq.ngrid) ier = -4
	      czz = complex(abs(cz)*dlz*0.5D0,0.0D0)
	    end if
	    c1 = cxx/dlxx-cx/dlx2
	    c2 = cxx/dlxx+cx/dlx2
	    c3 = cyy/dlyy-cy/dly2
...
c
      if (ier .ne. -4) then
	if (cemax.eq.0.0D0.and.alfmax.eq.0.0D0) then
...
c
	...
	do j=1,ny
	  do k=1,nz
	    do l=1,7
	      cof(i,j,k,l) = (0.0D0,0.0D0)
	    end do
	    cof(i,j,k,7) = (1.0D0,0.0D0)
	  end do
	end do
      end if
      if (nxb.eq.1) then
...
c
      implicit none
      integer nx,ny,nz
      complex(kind=2) phi(0:nx+1,0:ny+1,0:nz+1),cof(nx,ny,nz,8)
      integer intl,nxa,nxb,nyc,nyd,nze,nzf,ixp,jyq,kzr,iex,jey,kez,
     +nfx,nfy,nfz,iguess,maxcy,method,meth2,nwork,lwork,itero,
     +kcycle,iprer,ipost,intpol
      real(kind=2) xa,xb,yc,yd,ze,zf,tolmax,relmax
     ...
     +kcycle,iprer,ipost,intpol
      ...
      common/cud3c/kpbgn(50),kcbgn(50),ktxbgn(50),ktybgn(50),
     +ktzbgn(50),nxk(50),nyk(50),nzk(50),ngrid,klevel,kcur,kps
      real(kind=2) x,y,z,dlx,dly,dlz,dlx2,dly2,dlz2,dlxx,dlyy,dlzz
      complex(kind=2) cxx,cyy,czz,cx,cy,cz,ce,alfa,gbdy,c1,c2,c3,c4,c5,
     +c6
